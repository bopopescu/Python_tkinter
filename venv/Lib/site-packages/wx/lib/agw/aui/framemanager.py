...
class AuiManager(wx.EvtHandler):
    ...
    **Layer** - A layer is akin to an onion. Layer 0 is the very center of the managed pane.
    ...
    def CreateNotebook(self):
        ...
        notebook.GetAuiManager().SetMainManager(self)
...
##        docks = self.SmartShrink(docks, AUI_DOCK_LEFT)
    ...
    def DoUpdate(self):
        ...
        # Only the main manager should create/destroy notebooks...
        if not self._mainManager:
            self.UpdateNotebook()
...
##                    totalPixsize -= tmpPane.min_size.y
    ...
    def OnTabBeginDrag(self, event):
        ...
        if self._mainManager:
            self._mainManager.OnTabBeginDrag(event)
    ...
    def OnTabEndDrag(self, event):
        ...
        if self._mainManager:
            self._mainManager.OnTabEndDrag(event)
        else:
            self.Update()
            event.Skip()
    ...
    def OnTabPageClose(self, event):
        ...
        if self._mainManager:
            self._mainManager.OnTabPageClose(event)
    ...
    def OnTabSelected(self, event):
        ...
        if self._mainManager:
            self._mainManager.OnTabSelected(event)
            return
    ...
    def GetNotebooks(self):
        ...
        if self._mainManager:
            return self._mainManager.GetNotebooks()
    ...
    def SetMainManager(self, manager):
        """
        Sets the main manager for an automatic :class:`~wx.lib.agw.aui.auibook.AuiNotebook`.

        :param `manager`: an instance of :class:`AuiManager`.
        """

        self._mainManager = manager
